...
def generate_class_range(start, end, g):
    """
    根据开始和结束的数字生成一个类别范围，并且减去偏移量 g。
    """
    return np.arange(start - g, end - g)

def generate_c(cd, g):
    """
    根据给定的 cd（session // 2）生成对应的类别 c。
    每个 cd 对应一段类别范围，cd 的增加会增加更多的类别。
    """
    # 每段长度为 4，起始位置依次增加 5
    ranges = [generate_class_range(60 + 5 * i, 64 + 5 * i, g) for i in range(cd + 1)]
    return np.concatenate(ranges)

def cd_acc_utils(da, num_base, num_session, way, session, g):
    acc_dict = {  # 初始化各类精度
        'all_acc': 0.0,
        'base_acc': 0.0,
        'novel_acc': 0.0,
    ...
    # 初始化每个 session 对应的精度
    for i in range(num_session):
        acc_dict[f'sess{i}_acc'] = None
    
    # session 为 0 时，只有基础类的精度
    if session == 0:
        avger = Averager()
        for i in range(num_base):
            if i in da:
        ...
        for i in range(session + 1):
            if i == 0:
                sess_cls = range(num_base)
                acc_dict[f'sess{i}_acc'] = get_aver(sess_cls, da)
            else:
                sess_cls = range(num_base + way * (i - 1), num_base + way * i)
                acc_dict[f'sess{i}_acc'] = get_aver(sess_cls, da)

        # 计算当前 session 精度
        if session % 2 == 1:  # session 为奇数
            cd = session // 2
            c = generate_c(cd, g)

            # 如果 cd == 0
            if cd == 0:
                all_cls = np.concatenate((np.arange(num_base), 
                                          np.arange(num_base + (session // 2) * way, 
                                                    num_base + (session // 2 + 1) * way)))
                novel_cls = np.arange(num_base + (session // 2) * way, 
                                      num_base + (session // 2 + 1) * way)
            else:
                all_cls = np.concatenate((np.arange(num_base - cd), c,
                                          np.arange(num_base + (session // 2) * way, 
                                                    num_base + (session // 2 + 1) * way)))
                novel_cls = np.concatenate((c,
                                          np.arange(num_base + (session // 2) * way, 
                                                    num_base + (session // 2 + 1) * way)))

            former_cls = range(num_base + way * (session // 2))
            cur_cls = np.arange(num_base + (session // 2) * way, 
                                num_base + (session // 2 + 1) * way)
            base_cls = range(num_base - cd)

        else:  # session 为偶数
            ci = session // 2
            c = generate_c(ci, g)

            all_cls = np.concatenate((np.arange(num_base - ci), c))
            former_cls = np.concatenate((np.arange(num_base), c))
            cur_cls = range(num_base + way * (session - 1), num_base + way * session)
            base_cls = range(num_base - ci)
            novel_cls = c

        # 计算所有类别、基础类别、新类别等的准确率
        acc_dict['all_acc'] = get_aver(all_cls, da)
        acc_dict['former_acc'] = get_aver(former_cls, da)
        acc_dict['cur_acc'] = get_aver(cur_cls, da)
        acc_dict['base_acc'] = get_aver(base_cls, da)
...
def plot_tsne(fc_proto, test_class, save_path, session):
    ts = TSNE(n_components=2, init='pca', random_state=0)
    reslut = ts.fit_transform(fc_proto.cpu().numpy())
    # 调用函数，绘制图像
    fig = plot_embedding(reslut, np.arange(test_class), 't-SNE proto of session {}'.format(session))
    # 显示图像
...
def plot_embedding(data, label, title):
    ...
    for i in range(data.shape[0]):
        # 在图中为每个数据点画出标签
        plt.text(data[i, 0], data[i, 1], str(label[i]), color=plt.cm.Set2(label[i] / len(label)),
