...
class NDS(Dataset):
    ...
    def __init__(self, root='./', phase='train', 
        ...
        self.all_train_df = pd.read_csv(f"data/nsynth/{args.dataset}-fs-meta/{args.dataset}-fs_train.csv")
        self.all_val_df = pd.read_csv(f"data/nsynth/{args.dataset}-fs-meta/{args.dataset}-fs_val.csv")
        self.all_test_df = pd.read_csv(f"data/nsynth/{args.dataset}-fs-meta/{args.dataset}-fs_test.csv")
        with open(f"data/nsynth/{args.dataset}-fs-meta/{args.dataset}-fs_vocab.json") as vocab_json_file:
    ...
    def SelectfromClasses(self, df, index, per_num=None):
        ...
        data_tmp = []
        targets_tmp = []
        # append a colomn in df--digit label
        dig_labels = []
        for i in range(len(df['instrument'])):
            dig_labels.append(self.label_to_ix[df['instrument'][i]])
        df['label'] = np.array(dig_labels)
        ...
        for i in index:
            ...
            for j in ind_cl:
                filename = os.path.join(df['audio_source'][j], 'audio', df['filename'][j] + '.wav')
